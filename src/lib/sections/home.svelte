...
<script lang="ts">
	...
	// DOM Node Binds for animations
	let homeContainerElement: HTMLElement; // Container
	let backgroundContainerElement: HTMLElement,
		backgroundImageElement: HTMLElement; // Offsets
	let titleWord1Element: HTMLElement,
		titleWord2Element: HTMLElement,
		shortDetailsElement: HTMLElement,
		callToActionElement: HTMLElement; // Elements for animations
	let signaturePath1: SVGPathElement,
		signaturePath2: SVGPathElement,
		signaturePath3: SVGPathElement,
		signaturePath4: SVGPathElement; // SVG Paths
	...
	onMount(async () => {
		try {
			...
			developerPosition = data.position;
			developerPlace = data.place;
			name = data.name;
			lastname = data.last;
		} catch (error) {
			console.error(error);
	...
	function introAnimations() {
		...
		anime({
			...
			delay: 500,
			complete: () => {
				backgroundContainerElement.style.boxShadow =
					"3px 9px 18px rgba(0, 0, 0, 0.2)";
			},
		});
		anime({
			targets: backgroundImageElement,
		...
		titleElements.forEach((e) => {
			e.style.transform = "translateY(160%) rotate(10deg)";
		});
		anime({
			targets: titleElements,
			rotate: "0",
...
<div
	...
	bind:this={homeContainerElement}
>
	<div class="content-wrapper">
		<div class="flex">
			<div class="flex-wrapper first">
			...
			<div class="flex-wrapper second">
				<h1 class="title">
					<div class="title-mask">
						<div class="word" bind:this={titleWord1Element}>
							Hemanshu
						</div>
					</div>
					<br />
					<div class="title-mask">
						<div class="word" bind:this={titleWord2Element}>
							Patil
						</div>
					</div>
				</h1>
				<div class="occupation mask">
					<p class="paragraph" bind:this={shortDetailsElement}>
						{developerPosition} <br /> {developerPlace}
					</p>
				</div>
				<div class="wrapper action-mask">
					<div class="action" bind:this={callToActionElement}>
			...
			<div
				class="parallax-wrapper home-back"
				bind:this={backgroundContainerElement}
			>
				{#await loadImage("assets/imgs/home-back.jpg") then src}
					<img
						{src}
